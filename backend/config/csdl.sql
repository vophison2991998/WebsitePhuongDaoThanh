-- =============================================================================
-- HỆ THỐNG QUẢN LÝ TẬP TRUNG HOÀN CHỈNH (V3 - CLEANSED)
-- =============================================================================

-- =========================================
-- 0. RESET DATABASE (CLEAN UP)
-- =========================================
-- Xóa tất cả các bảng chính và phụ thuộc
...
-- =========================================
-- 1. CẤU TRÚC PHÂN QUYỀN (RBAC) & CORE
-- =========================================
CREATE TABLE roles (
...
-- =========================================
-- 3. CÁC NGHIỆP VỤ (TRANSACTIONS)
-- =========================================

-- 3.1. QUẢN LÝ CÔNG VIỆC (TASKS & LOGS)
CREATE TABLE tasks (
    id SERIAL PRIMARY KEY,
...
-- =========================================
-- 5. INSERT DATA (DỮ LIỆU THỬ NGHIỆM)
-- =========================================
...
-- =========================================
-- 6. QUERIES (DÙNG CHO TRANG DASHBOARD/REPORT)
-- =========================================

-- Q1: Tổng Tồn Kho Hiện Tại
SELECT
    wp.name AS product_name,
    wp.unit,
    SUM(l.current_quantity) AS total_stock
FROM lot l
JOIN water_product wp ON l.product_id = wp.product_id
GROUP BY wp.name, wp.unit
HAVING SUM(l.current_quantity) > 0;

-- Q2: Lịch sử Giao dịch (Tất cả Nhận & Trả)
SELECT
    t.transaction_code,
    t.type,
    t.transaction_date,
    ...
    up.full_name AS processed_by
FROM "transaction" t
LEFT JOIN party p ON t.party_id = p.party_id
LEFT JOIN users u ON t.created_by = u.id
LEFT JOIN user_profiles up ON u.id = up.user_id
ORDER BY t.transaction_date DESC;

-- Q3: Trừ tồn kho thành công (Kiểm tra lại lot LOT-005)
SELECT lot_id, batch_code, received_quantity, current_quantity 
FROM lot 
WHERE batch_code = 'LOT-005';